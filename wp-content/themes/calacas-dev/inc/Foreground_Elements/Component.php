<?php
/**
 * WP_Rig\WP_Rig\Foreground_Elements\Component class
 *
 * @package wp_rig
 */

namespace WP_Rig\WP_Rig\Foreground_Elements;
	...
	public function display_foreground_elements( $elements, $animations = array(), $styles = true ) {
		...
		if ( $elements ) :
			...
			foreach ( $elements as $key => $element ) :
				...
				$el_animations = wp_rig()->get_scroll_animation( $element['el_name'], $animations );
				...
				if ( 'image' === $element['acf_fc_layout'] ) :
					...
					<figure id="<?php echo esc_attr( $el_id ); ?>" class="block-element image-foreground" aria-hidden="true">
						...
						wp_rig()->set_scroll_animation( $el_animations );
			...
			if ( $styles ) :
				...
				<style>
					<?php
					echo wp_rig()->get_elements_custom_styles( $elements ); // phpcs:ignore
					?>
				</style>
				<?php

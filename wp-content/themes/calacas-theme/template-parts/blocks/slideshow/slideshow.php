...
?>
	...
	if ( ! empty( $slides ) ) :
		...
		foreach ( $slides as $slide_key => $slide ) :
			...
			<div class="slide-wrapper">
				...
				calacas_theme()->display_slide_options(
					array(
				...
				if ( $add_foreground_elements && ! empty( $elements ) ) :
					calacas_theme()->display_foreground_elements( $elements, $animations, false );
					$elements_custom_styles .= '
					' . calacas_theme()->get_elements_custom_styles( $elements, $animations ) . '
					';
				endif;
				...
				?>
					...
					calacas_theme()->display_content_container_options( $content_container_options );
					?>
									...
									?>
										...
										if ( $button || $second_button ) :
											...
											<div class="block-buttons mt-3">
												<?php
												if ( $button ) :
													$button_style = calacas_theme()->arr2str( $button['button_style'] );
													$button_style = $button_style ? $button_style : 'btn-primary';
													$button_size  = calacas_theme()->arr2str( $button['button_size'] );
													calacas_theme()->get_button( $button['button_link'], $button_style, $button_size );
												endif;

												if ( $second_button ) :
													$second_button_style = calacas_theme()->arr2str( $second_button['button_style'] );
													$second_button_style = $second_button_style ? $second_button_style : 'btn-outline-primary';
													$second_button_size  = calacas_theme()->arr2str( $second_button['button_size'] );
													calacas_theme()->get_button( $second_button['button_link'], $second_button_style, $second_button_size );
												endif;
												?>
											</div>
											<?php
				...
				$slide_custom_styles .= '
				' . calacas_theme()->get_slide_custom_styles( $slide ) . '
				';
				?>
				...
				foreach ( $slides as $slide_key => $slide ) :
					...
					<div class="slide-thumb">
						<div class="slide-thumb-wrapper <?php echo esc_attr( $slide_thumb_class ); ?>">
							...
							if ( $headline || $tagline ) :
								// Print Tagline.
								calacas_theme()->get_tagline( $tagline, 'fs8 text-uppercase letter-space-2 my-2 block' );
								calacas_theme()->get_headline( $headline, 'h6', 'div' );
							endif; // Ends if has headings.
							?>
							</div>
						</div>
	...
	calacas_theme()->initiate_carousel_script( $block_id, $block_slug, $carousel_target, $options, $thumbs_target );
	...
	if ( ! empty( $slides ) && $options['add_thumbnails'] ) :
		...
		calacas_theme()->initiate_carousel_script( $block_id, $block_slug, $thumbs_target, $thumbs_options, $carousel_target );
	endif;
	?>
	...
	if ( ! empty( $slides ) && $options['add_thumbnails'] ) {
		...
		if ( $options['thumb_background_color'] ) {
			$thumb_background_color = 'background-color: ' . calacas_theme()->hex2rgba( esc_attr( $options['thumb_background_color'] ), '0.25' ) . ';';
		}
	...
	calacas_theme()->display_block_custom_styles( $block, $carousel_custom_styles );
	?>

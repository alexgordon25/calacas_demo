...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'entry' . $post_class ); ?>>
	<div class="item">
		...
		if ( is_singular( get_post_type() ) ) :
			...
			<p class="h7 text-primary-gray text-uppercase letter-space-2"><?php echo esc_html__( 'Artist profile', 'calacas-theme' ); ?></p>
...
if ( is_singular( get_post_type() ) ) {
	...
	if ( 'post' === get_post_type() || get_post_type_object( get_post_type() )->has_archive ) {
		the_post_navigation(
			[
				'prev_text' => '<div class="post-navigation-sub"><span>' . esc_html__( 'Previous:', 'calacas-theme' ) . '</span></div>%title',
				'next_text' => '<div class="post-navigation-sub"><span>' . esc_html__( 'Next:', 'calacas-theme' ) . '</span></div>%title',
			]
		);
	}

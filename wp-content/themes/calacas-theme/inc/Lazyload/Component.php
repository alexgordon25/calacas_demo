<?php
/**
 * WP_Calacas\WP_Calacas\Lazyload\Component class
 *
 * @package calacas_theme
 */

namespace WP_Calacas\WP_Calacas\Lazyload;
...
class Component implements Component_Interface {
	...
	public function action_customize_register_lazyload( WP_Customize_Manager $wp_customize ) {
		$lazyload_choices = array(
			'lazyload'    => __( 'Lazy-load on (default)', 'calacas-theme' ),
			'no-lazyload' => __( 'Lazy-load off', 'calacas-theme' ),
		);
		...
		$wp_customize->add_control(
			'lazy_load_media',
			array(
				'label'           => __( 'Lazy-load images', 'calacas-theme' ),
				'section'         => 'theme_options',
				'type'            => 'radio',
				'description'     => __( 'Lazy-loading images means images are loaded only when they are in view. Improves performance, but can result in content jumping around on slower connections.', 'calacas-theme' ),
	...
	public function action_enqueue_lazyload_assets() {
		wp_enqueue_script(
			'calacas-theme-lazy-load-images',
			get_theme_file_uri( '/assets/js/lazyload.min.js' ),
			array(),
			calacas_theme()->get_asset_version( get_theme_file_path( '/assets/js/lazyload.min.js' ) ),
			false
		);
		wp_script_add_data( 'calacas-theme-lazy-load-images', 'defer', true );
		wp_script_add_data( 'calacas-theme-lazy-load-images', 'precache', true );
	}

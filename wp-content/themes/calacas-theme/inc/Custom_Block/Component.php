<?php
/**
 * WP_Calacas\WP_Calacas\Custom_Block\Component class
 *
 * @package calacas_theme
 */

namespace WP_Calacas\WP_Calacas\Custom_Block;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function register_acf_block_types() {
		...
		acf_register_block_type(
			array(
				'name'              => 'hero',
				'title'             => __( 'Hero', 'calacas-theme' ),
				'description'       => __( 'Custom Hero block.', 'calacas-theme' ),
		...
		acf_register_block_type(
			array(
				'name'              => 'Slideshow',
				'title'             => __( 'Slideshow', 'calacas-theme' ),
				'description'       => __( 'Custom Slideshow block.', 'calacas-theme' ),
				'render_callback'   => array( $this, 'acf_block_callback' ),
				'category'          => 'calacas-blocks',
	...
	public function display_block_options( $args = array() ) {
		...
		// Get the block ID.
		$block_id = calacas_theme()->get_block_id( $args['block'] );
		...
		if ( $args['background_type'] ) :
			...
			$bg_animations = calacas_theme()->get_scroll_animation( 'background', get_field( 'animations' ) );
	...
	public function display_block_custom_styles( $block, $additional_styles = '' ) {
		...
		$the_block_id               = calacas_theme()->get_block_id( $block );
	...
	public function register_acf_color_palette() {
		...
		$color_palette = calacas_theme()->get_the_colors();
	...
	public function enqueue_foreground_video_scripts() {
		...
		if ( file_exists( get_theme_file_path( $js_ytplayer_path ) ) ) {
			wp_enqueue_script(
				...
				array( 'jquery' ),
				calacas_theme()->get_asset_version( get_theme_file_path( $js_ytplayer_path ) ),
				false
			);
		}
		...
		if ( file_exists( get_theme_file_path( $js_foreground_video_path ) ) ) {
			wp_enqueue_script(
				...
				calacas_theme()->get_asset_version( get_theme_file_path( $js_foreground_video_path ) ),
				false
			);
		}
	...
	public function enqueue_carousel_scripts() {
		...
		if ( file_exists( get_theme_file_path( $js_slick_path ) ) ) {
			wp_enqueue_script(
				...
				array( 'jquery' ),
				calacas_theme()->get_asset_version( get_theme_file_path( $js_slick_path ) ),
				true
			);
		}

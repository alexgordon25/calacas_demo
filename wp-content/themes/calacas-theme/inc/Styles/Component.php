<?php
/**
 * WP_Calacas\WP_Calacas\Styles\Component class
 *
 * @package calacas_theme
 */

namespace WP_Calacas\WP_Calacas\Styles;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'calacas-theme-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = calacas_theme()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'calacas-theme-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'calacas-theme' ), $handle ) ), 'Calacas WP Custom Theme 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'calacas_theme_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! calacas_theme()->is_amp();
		 ...
		 */
		return apply_filters( 'calacas_theme_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = array(
			'calacas-theme-global'     => array(
				'file'   => 'global.min.css',
				'global' => true,
			),
			// 'calacas-theme-comments'   => array(
			// 	'file'             => 'comments.min.css',
			// 	'preload_callback' => function() {
			// 		return ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() );
			// 	},
			// ),
			'calacas-theme-content'    => array(
				'file'             => 'content.min.css',
				'preload_callback' => '__return_true',
			),
			// 'calacas-theme-sidebar'    => array(
			// 	'file'             => 'sidebar.min.css',
			// 	'preload_callback' => function() {
			// 		return calacas_theme()->is_primary_sidebar_active();
			// 	},
			// ),
			// 'calacas-theme-widgets'    => array(
			// 	'file'             => 'widgets.min.css',
			// 	'preload_callback' => function() {
			// 		return calacas_theme()->is_primary_sidebar_active();
			// 	},
			// ),
			// 'calacas-theme-front-page' => array(
			...
			// ),
			'wp-ytplayer' => array(
				'file'             => 'libs/YTPlayer/jquery.mb.YTPlayer.min.css',
			),
			'wp-foreground-video' => array(
				'file'             => 'foreground-video.min.css',
			),
		);
		 ...
		 */
		$css_files = apply_filters( 'calacas_theme_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'calacas_theme_google_fonts', $google_fonts );
	...
	protected function get_typekit_fonts_url() : string {
		$typekit_fonts = 'https://use.typekit.net/klj5qzy.css';

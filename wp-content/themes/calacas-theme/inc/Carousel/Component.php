<?php
/**
 * WP_Calacas\WP_Calacas\Carousel\Component class
 *
 * @package calacas_theme
 */

namespace WP_Calacas\WP_Calacas\Carousel;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function display_slide_options( $args = array() ) {
		...
		if ( $slide['background_type'] ) :
			...
			$bg_animations = calacas_theme()->get_scroll_animation( 'background', $slide['animations'] );
			...
			if ( 'classic' === $slide['background_type'] && $background_image ) :
				...
				?>
					<figure class="block-background image-background" aria-hidden="true">
						...
						calacas_theme()->set_scroll_animation( $bg_animations );
			...
			if ( 'video' === $slide['background_type'] && $background_video ) :
				...
				$background_video_alt = $background_video_title ? sprintf( 'Video Background of %s', 'calacas-theme', esc_attr( $background_video_title ) ) : __( 'Video Background', 'calacas-theme' );
	...
	public function get_slide_custom_styles( $slide ) {
		...
		$gradient_position          = calacas_theme()->arr2str( $slide['gradient_position'] );
		$background_object_fit      = calacas_theme()->arr2str( $slide['background_object_fit'] );
		$background_object_position = calacas_theme()->arr2str( $slide['background_object_position'] );
		$overlay_type               = $slide['overlay_type'];
		...
		$overlay_gradient_position  = calacas_theme()->arr2str( $slide['overlay_gradient_position'] );
		$overlay_opacity            = $slide['overlay_opacity'];
		$overlay_blend_mode         = calacas_theme()->arr2str( $slide['overlay_blend_mode'] );
		...
		if ( $options['md_responsive'] ) {
			...
			$md_responsive = '
			{
				...
				settings: {
					...
					autoplay: ' . calacas_theme()->bool( $options['md_autoplay'] ) . ',
					infinite: ' . calacas_theme()->bool( $options['md_infinite'] ) . ',
					dots: ' . calacas_theme()->bool( $options['md_dots'] ) . ',
					arrows: ' . calacas_theme()->bool( $options['md_arrows'] ) . ',
				}
			} ' . $join;
		}
		...
		if ( $options['sm_responsive'] ) {
			$sm_responsive = '
			{
				...
				settings: {
					...
					autoplay: ' . calacas_theme()->bool( $options['sm_autoplay'] ) . ',
					infinite: ' . calacas_theme()->bool( $options['sm_infinite'] ) . ',
					dots: ' . calacas_theme()->bool( $options['sm_dots'] ) . ',
					arrows: ' . calacas_theme()->bool( $options['sm_arrows'] ) . ',
				}
			}
			';
		}
		...
		jQuery( function( $ ) {
			...
			var initialize_' . $var_id . ' = function( $block ) {
				$block.find( "' . $carousel_target . '" ).slick({
					...
					autoplay: ' . calacas_theme()->bool( $options['autoplay'] ) . ',
					infinite: ' . calacas_theme()->bool( $options['infinite'] ) . ',
					dots: ' . calacas_theme()->bool( $options['dots'] ) . ',
					arrows: ' . calacas_theme()->bool( $options['arrows'] ) . ',
					...
					adaptiveHeight: ' . calacas_theme()->bool( $options['adaptiveheight'] ) . ',
					swipe: ' . calacas_theme()->bool( $options['swipe'] ) . ',
					draggable: ' . calacas_theme()->bool( $options['draggable'] ) . ',
					centerMode: ' . calacas_theme()->bool( $options['centermode'] ) . ',
					centerPadding: "' . $options['centerpadding'] . 'px",
					fade: ' . calacas_theme()->bool( $options['fade'] ) . ',
					initialSlide: ' . esc_attr( $initialslide ) . ',
					lazyLoad: "' . $options['lazyload'] . '",
					pauseOnFocus: ' . calacas_theme()->bool( $options['pauseonfocus'] ) . ',
					pauseOnHover: ' . calacas_theme()->bool( $options['pauseonhover'] ) . ',
					focusOnSelect: true,
					rows: ' . $options['rows'] . ',
					' . $slides_per_row . '
					' . $thumbnail_sync . '

<?php
/**
 * WP_Calacas\WP_Calacas\Animations\Component class
 *
 * @package calacas_theme
 */

namespace WP_Calacas\WP_Calacas\Animations;
	...
	public function action_enqueue_animations_script() {
		...
		if ( get_field( 'animations_toggle', $post->ID ) ) {
			...
			wp_enqueue_script(
				...
				array(),
				calacas_theme()->get_asset_version( get_theme_file_path( '/assets/js/libs/lax.min.js' ) ),
				false
			);
			wp_enqueue_script(
				'wp-animations',
				...
				array( 'wp-lax' ),
				calacas_theme()->get_asset_version( get_theme_file_path( '/assets/js/animations.min.js' ) ),
				false
			);
